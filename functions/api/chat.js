...
const SICADA_TERMS = [
  "sicada", "your company", "your services", "your solution", "your team", "what do you do", "what can you do", "services", "capabilities", "case study", "case studies", "industry", "industries", "contact", "consultation",
];

const BUSINESS_TERMS = [
  "our company", "our business", "our organization", "we need", "we want", "we are planning", "we're planning", "existing crm", "existing erp", "crm", "erp", "ai", "artificial intelligence", "machine learning", "ml", "llm", "rag", "agent", "automation", "automate", "prediction", "predictive", "computer vision", "cybersecurity", "security", "threat detection", "fraud", "cloud", "data engineering", "integration", "integrate", "software", "application", "app", "workflow", "process", "modernize", "modernise", "digital transformation",
];

const CODE_TERMS = [
  "write code", "generate code", "give me code", "source code", "python code", "javascript code", "typescript code", "java code", "react code", "sql query", "debug this code", "fix this code", "program this", "coding problem", "leetcode", "```",
];

const GENERAL_OFF_TOPIC_TERMS = [
  "weather", "president", "prime minister", "cricket", "football", "movie", "song", "recipe", "joke", "astrology", "horoscope", "homework", "math problem", "translate this", "write an essay", "write a poem", "write a story",
];

const PROMPT_INJECTION_TERMS = [
  "ignore previous instructions", "ignore your instructions", "system prompt", "developer message", "reveal your prompt", "show your prompt", "jailbreak", "act as", "pretend you are", "new role",
];
...
function nextStepForIntent(intent) {
  if (intent === "high") {
    return { label: "Talk to Sicada", href: "/contact", message: "This sounds like a concrete project. The next useful step is to share your current system, desired outcome and any important constraints with the Sicada team." };
  }
  if (intent === "medium") {
    return { label: "Explore AI solutions", href: "/services", message: "You may benefit from reviewing Sicada's AI capabilities before discussing a specific implementation." };
  }
  return null;
}
...
async function getGroqReply(apiKey, message, history) {
  ...
  return validateModelReply(data?.choices?.[0]?.message?.content);
}
...
export async function onRequestPost(context) {
  try {
    ...
    if (domain === "OUT_OF_SCOPE") {
      return Response.json({ reply: OUT_OF_SCOPE_REPLY, source: "sicada-domain-gate", model: null, domain, leadIntent: "low", nextStep: null, blocked: true });
    }

    const leadIntent = detectLeadIntent(message, history);
    const nextStep = nextStepForIntent(leadIntent);
    let reply = null;
    let source = "sicada-curated";
    let model = null;

    try {
      reply = await getGroqReply(context.env?.GROQ_API_KEY, message, history);
      if (reply) {
        source = "groq";
        model = GROQ_MODEL;
      }
    ...
    if (!reply) reply = localAnswer(message, history);

    return Response.json({ reply, source, model, domain, leadIntent, nextStep, blocked: false });
  } catch {
